...
A RaFTS workflow relies on a suite of YAML configuration files to dictate file I/O, dataset schemas, and algorithm hyperparameters. Different datasets/workflows have different sets of config files, as stored inside `scripts/workflow_configs/{workflow_subdir}/`. All config files for a given workflow must be in the same directory!
