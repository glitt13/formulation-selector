...
def read_df_ext(path_to_file: str | os.PathLike) -> pd.DataFrame:
    ...
    if '{' in str(path_to_file):
        logging.error("The following path still contains f-string formatting & needs rectified:\n %s", path_to_file)
    ...
    if 'csv' in path_to_file.suffix:
        ...
        logging.error("Expecting path to file containing comids to be csv, parquet, or gpkg file")
...
def _get_comids_std_attrs(path_attr_config: str | os.PathLike, 
    ...
    :param loc_id_cols: List of possible location ID column names (aka comid column) in the metadata 
      tabular file, defaults to ['featureID','comid'], listed in order of preference.
    ...
    for ds in datasets: # ds likely used for f-string eval with path_meta
        for ds_type in likely_ds_types: # ds_type likely used for f-string eval with path_meta
            ...
            if path_meta.exists():
                logging.info("Reading %s", path_meta)
                ...
                if len(loc_id_col) != 1:
                    logging.error("Could not find any of the location ID column names in the attribute metadata file %s. Expected colnames: %s", path_meta, ' or '.join(loc_id_cols))
    ...
    if len(ls_comids_attrs) == 0:
        logging.warning("Unexpectedly, no data found reading standardized metadata generated by basin attribute grabbing workflow.")
...
def io_std_attrs(df_new_vars: pd.DataFrame,
    ...
    if df_new_vars.shape[0] > 0:
        ...
        if path_tfrm_comid.exists():
            logging.info("Updating %s", path_tfrm_comid)
            ...
            df_new_vars = raftsutil._check_attr_rm_dupes(df_new_vars, ascending = False)
        else:
...
def _gen_tform_df(all_attr_ddf: dd.DataFrame, new_var_id: str,
    ...
    if all_attr_ddf['featureID'].nunique().compute() != 1:
        logging.error("Only expecting one unique location identifier. Reconsider first row logic.")
...
def write_missing_attrs(attrs_retr_sub:list, dir_db_attrs: str | os.PathLike,
    ...
    df_all = raftsutil.rafts_read_attr_comid(dir_db_attrs, comids_resp=[str(comid)], attrs_sel='all',
    ...
    if df_all.shape[0]>0:
        ...
        logging.info("Absolutely no attribute data found for comid %s. Acquire it!", comid)
    ...
    df_need_attrs_comid.to_csv(path_need_attrs, mode = 'a',
                                ...
                                index=False)
    logging.info("Wrote needed comid-attributes to \n%s", path_need_attrs)
...
def tfrm_attr_comids_wrap(comids: Iterable, path_tfrm_cfig: str | os.PathLike):
    ...
    path_attr_config=raftsutil.build_cfig_path(path_tfrm_cfig, Path(fio.get('name_attr_config')))
    attr_cfig = raftsutil.AttrConfigAndVars(path_attr_config) 
    attr_cfig._read_attr_config()
    ...
    for comid in comids:
        ...
        if ddf_loc_attrs is None:
            ...
            logging.warning(warn_msg_missing)
        ...
        for new_var in cstm_vars_need: 
            if len(cstm_vars_need) != len(dict_need_vars_funcs.get('funcs')):
                logging.error("DO NOT PROCEED! Double check assumptions around fta._id_need_tfrm_attrs indexing")
            ...
            try:
                df_attr_sub = raftsutil.rafts_read_attr_comid(dir_db_attrs, comids_resp=[str(comid)], attrs_sel=attrs_retr_sub,
                ...
                logging.warning('Missing attributes include %s', "|".join(attrs_retr_sub))
                continue
            ...
            if df_attr_sub.shape[0] < len(attrs_retr_sub):
                ...
                if fio.get('path_rafts_attrs_miss'):
                    # Path to the Rscript, requires proc.attr.hydfab package to be installed!
                    path_rafts_attrs_miss = fio.get('path_rafts_attrs_miss').format(home_dir = home_dir)
                    args = [str(path_attr_config)]
                    try:
                        logging.info("Attempting to retrieve missing attributes using %s", Path(path_rafts_attrs_miss).name)
                        result = subprocess.run(['Rscript', path_rafts_attrs_miss] + args, capture_output=True, text=True)
                        logging.info(result.stdout) # Print the output from the Rscript
                        logging.error(result.stderr)  # If there's any error output
                    except:
                        logging.error("Could not run the Rscript %s. Ensure proc.attr.hydfab R package installed and appropriate path to rafts_attrs_miss.R", path_rafts_attrs_miss)
                    # Re-run the attribute retrieval in case new ones now available
                    raftsutil.rafts_read_attr_comid(dir_db_attrs, comids_resp=[str(comid)], attrs_sel=attrs_retr_sub,
                                _s3 = None,storage_options=None,read_type='filename')
                continue
            ...
            if any(pd.isnull(attr_val)):
                ...
                if any(pd.isnull(need_vals)):
                    logging.warning("Null values returned for %s with featureID %s.", attrs_retr_sub, comid)
                else:
                    logging.error("Unexpected NULL value returned after aggregating and transforming attributes. Inspect %s with featureID %s", new_var, comid)
    ...
    if path_need_attrs.exists():
        logging.info("Dropping any duplicate entries in %s", path_need_attrs)
