...
@usage: python rafts_perf_viz.py "/full/path/to/viz_config.yaml"
...
if __name__ == "__main__":
    ...
    path_viz_config = Path(args.path_viz_config).expanduser() #Path(f'~/FSDS/rafts/scripts/workflow_configs/legacy/xssa/xssa_viz_config.yaml') 
    ...
    path_pred_config = raftsutil.build_cfig_path(path_viz_config,viz_cfg.get('name_pred_config',None)) # currently, this gives the pred config path, not the attr config path
    ...
    path_main_config = raftsutil.build_cfig_path(path_viz_config,f'{prefix_viz}_config.yaml')
    ...
    # Enforce style
    style_path = pkg_resources.resource_filename('rafts_algo', 'RaFTS_theme.mplstyle')
    plt.style.use(style_path)

    # Loop through all datasets
    for ds in datasets:
        # path_meta_pred = f'{path_meta_pred}'.format(ds = ds, dir_std_base = dir_std_base, ds_type = ds_type, write_type = write_type)
        path_meta_pred = raftsutil.build_pred_locs_path(path_meta_template=path_meta_pred, dir_std_base=dir_std_base, 
        ...
        for algo in algos:
            ...
            for metric in metrics:
                ...
                path_pred = raftsutil.std_pred_path(dir_out,algo=algo,metric=metric,dataset_id=ds)
                ...
                if 'pred_map' in true_keys:
                    states = raftsplot.gen_conus_basemap(f'{dir_out}/data_visualizations/')
                    ...
                    raftsplot.plot_map_pred(geo_df=geo_df, states=states, 
                                        title=f'RaFTS Predicted Performance Map: {ds}', 
                                        metr=metric, colname_data='performance')

                    # Save the plot as a .png file
                    output_path = raftsplot.std_map_pred_path(dir_out_viz_base=dir_out_viz_base,
                ...
                if 'obs_vs_sim_scatter' in true_keys:
                    ...
                    # obs = raftsutil._open_response_data_rafts(dir_std_base=dir_std_base, ds=ds)
                    ...
                    raftsplot.plot_pred_vs_obs_regr(y_pred=data['prediction'], y_obs=data[metric], 
                                                ds = ds, metr=metric)

                    # Save the plot as a .png file
                    output_path = raftsplot.std_regr_pred_obs_path(dir_out_viz_base=dir_out_viz_base,
                                                               ds=ds, metr=metric, algo_str=algo,
                                                            split_type='prediction')
                    plt.savefig(output_path, dpi=300, bbox_inches='tight')
                    plt.clf()
