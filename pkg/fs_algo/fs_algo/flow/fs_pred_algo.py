...
if __name__ == "__main__":
    ...
    for ds in datasets:
        ...
        if not name_prep_config:
            ...
            #vpu_id_col=fio.get('vpu_id_col')
            df_hfatlas = fsutil.read_hfatlas_wrap(path_hfatl=path_pred_locs, attrs_sel=attrs_sel, 
                              map_id_col=map_id_col)
            df_attr = fsutil.hfatl_std_long(df_hfatlas,map_id_col)
            df_attr['featureID'] = df_attr[map_id_col]
            df_attr['featureSource'] = fio.get('featureSource')
