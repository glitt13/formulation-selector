...
if __name__ == "__main__":
    ...
    for ds in datasets:
        ...
        if not name_prep_config:
            ...
            try: # Allow user-defined locations of interest, otherwise set comids_resp = None
                locids_pred = fsutil._read_pred_comid(path_pred_locs, comid_pred_col)
            except: # This means all locations with all required attribute date inside dir_db_attrs will be considered
                logging.info(f"The {path_pred_locs} and/or comid_pred_col {comid_pred_col} are undefined, causing the previous error message.")
                logging.info(f"Predicting across all locations in files inside {dir_db_attrs} with required attribute data")
                locids_pred = None
            
            new_df_attr = fsutil.fs_read_attr_comid(dir_db_attrs=dir_db_attrs,
                                      comids_resp = locids_pred, attrs_sel = attrs_sel,
                                      read_type = 'all')
        ...
        for resp_var in resp_vars:
            ...
            for algo in algos:
                ...
                logging.info(f"Wrote predictions to {path_pred_out}")
