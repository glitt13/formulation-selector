...
:usage: 
uv run python fs_agg_nexus_hfatl.py --path_attr_config "nex_test_attr_config.yaml"
uv run python fs_agg_nexus_hfatl.py --path_pred_config "nex_test_pred_config_hf22.yaml"
"""
...
if __name__ == "__main__":
    ...
    if args.path_pred_config:
        ...
        path_attr_config = Path(fsutil.build_cfig_path(path_pred_config, name_attr_config))
    else:
        run_mode = 'training'
    ...
    path_prep_config = fsutil.build_cfig_path(path_attr_config, name_prep_config)
    ...
    if run_mode == 'prediction':
        ...
        resolved_pred = {k: fsutil.resolve_fstrings(v, resolve_dict) if isinstance(v, str) else v for k, v in pred_config.items()}
