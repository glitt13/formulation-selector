  ...
  if(base::length(idxs_na_id) > 0){  # assumed to be oCONUS if comid unavailable
    ...
    if(!base::is.null(Retr_Params$paths$path_oconus_hfab_config)){
      dt_hfuid <- proc.attr.hydfab::retr_hfuids(loc_ids=loc_ids,
              path_oconus_hfab_config=Retr_Params$paths$path_oconus_hfab_config, # e.g. "~/git/formulation-selector/scripts/workflow_configs/legacy/bm_test25/bm_oconus_config.yaml"
              featureSource = featureSource)
...
gen_ds_gpkg <- function(dir_db_gpkg, path_save_gpkg, gage_ids,epsg=4326){
  ...
  #' @seealso \link[proc.attr.hydfab]{grab_attrs_datasets_fs_wrap}
