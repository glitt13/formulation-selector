...
attr_cfig_parse <- function(path_attr_config){
  ...
  # 2025-07-22 add dir_db_gpkg, make dirs that don't exist, GL
  ...
  dir_std_base <- glue::glue(base::unlist(raw_config$file_io)[['dir_std_base']]) #file.path(dir_base,"input","user_data_std") # The location of standardized data generated by rafts_prep python package
  ...
  dir_db_gpkg <- base::file.path(base::dirname(dir_db_attrs), "gpkg")
  ...
  # --- START: Robust Dataset Resolution ---
  datasets <- c()

  # 1. Safely extract the prep config name from the file_io list
  name_prep_config <- tryCatch({
    base::unlist(raw_config$file_io)[['name_prep_config']]
  }, error = function(e) { NULL })

  # 2. If a prep config is linked, parse it to extract the dataset_name
  if (!base::is.null(name_prep_config) && !base::is.na(name_prep_config)) {
    path_prep_config <- base::file.path(base::dirname(path_attr_config), name_prep_config)

    if (base::file.exists(path_prep_config)) {
      prep_config <- yaml::read_yaml(path_prep_config)

      if (!base::is.null(prep_config$formulation_metadata)) {
        names_prep_meta <- base::unlist(base::lapply(prep_config$formulation_metadata, base::names))
        idx <- base::which(names_prep_meta == "dataset_name")

        if (base::length(idx) > 0) {
          dataset_name <- prep_config$formulation_metadata[[idx[1]]][['dataset_name']]
          if (!base::is.null(dataset_name)) {
            datasets <- base::c(dataset_name) # Wrap in vector for downstream loops
          }
        }
      }
    } else {
      logr::log_print(glue::glue("Prep config {path_prep_config} not found. Falling back to attr config."), level="WARN")
    }
  }

  # 3. DURABLE FALLBACK: Legacy behavior if no prep config is found or dataset_name is missing
  if (base::length(datasets) == 0) {
    tryCatch({
      names_form_meta <- base::unlist(
        base::lapply(raw_config$formulation_metadata, function(x) base::names(x))
      )
      idx_ds <- base::which(names_form_meta == "datasets")

      if (base::length(idx_ds) > 0) {
        datasets <- raw_config$formulation_metadata[[idx_ds[1]]][['datasets']]
      }
    }, error = function(e) {
      logr::log_print("Could not resolve dataset names from legacy formulation_metadata block.", level="WARN")
    })
  }

  # 4. Final safety check
  if (base::length(datasets) == 0 || base::is.null(datasets)) {
    stop_msg <- "Could not resolve dataset names from either the prep config or attribute config."
    logr::log_print(stop_msg, level="ERROR")
    stop(stop_msg)
  }
  # --- END: Robust Dataset Resolution ---
  ...
  # ensure directories exist,also present in proc.attr.hydfab::proc_attr_gageids
...
build_cfig_path <- function(path_known_config, path_or_name_cfig) {
  ...
  #' @seealso \code{rafts_algo.build_cfig_path} The python equivalent of this function
  #' @export
...
read_rafts_attr_menu_config <- function(){
  ...
  path_attr_menu <- base::file.path(dir_extdata, "rafts_attr_menu.yaml")
...
map_attrs_to_dataset <- function(vars){
  ...
  # dir_extdata <- system.file("extdata",package="proc.attr.hydfab")
  # path_attr_menu <- base::file.path(dir_extdata, "rafts_attr_menu.yaml")
  ls_attr_menu <- proc.attr.hydfab:::read_rafts_attr_menu_config()
...
std_path_gpkg_db <- function(dir_db_gpkg){
  #' @title The filepath to the large gpkg database containing all locations
  #' ever acquired when running RaFTS
  #' @param dir_db_gpkg The directory of the geopackage database
  #' @seealso  \link[proc.attr.hydfab]{attr_cfig_parse} creates the directory
  #' structure for \code{dir_db_gpkg}
  #' @export
  ...
  path_gpkg_all <- base::file.path(dir_db_gpkg,
                                   "all_locs.gpkg")
  return(path_gpkg_all)
}

std_path_retr_gpkg <- function(path_rafts_prep){
  ...
  #'. \code{rafts_algo.utils._std_rafts_prep_ds_companion_gpkg_path}
  #' @param path_rafts_prep Path used for the standardized dataset created using
  #' \code{rafts_prep.proc_eval_metrics.proc_col_schema}
  #' @seealso \code{rafts_algo.utils._std_rafts_prep_ds_companion_gpkg_path}
  #' @seealso  \link[proc.attr.hydfab]{read_rafts_retr_gpkg}
  #' @seealso \link[proc.attr.hydfab]{std_dir_dataset}
  #' @export
  path_rafts_prep <- fs::path(path_rafts_prep)
  sub_name_loc <- fs::path_ext_remove(path_rafts_prep)
  ...
  path_gpkg_rafts_prep <- fs::path(fs::path_dir(path_rafts_prep), new_name_gpkg)
...
read_rafts_retr_gpkg <- function(path_save_gpkg, verbose = FALSE){
  ...
  #'  and created by \link[proc.attr.hydfab]{rafts_retr_nhdp_comids_geom_wrap}
  #' @details The renaming may be a temporary solution just-in case file
  #' originated in python \code{rafts_algo}
  #' @param path_save_gpkg filepath to the geopackage
  #' @seealso \link[proc.attr.hydfab]{rafts_retr_nhdp_comids_geom_wrap}
  #' @seealso \link[proc.attr.hydfab]{std_path_retr_gpkg}
  #' @seealso \link[proc.attr.hydfab]{rafts_retr_nhdp_comids_geom_wrap}
  #' @export
...
rafts_retr_nhdp_comids_geom_wrap <- function(path_save_gpkg,
  ...
  #' @seealso \link[proc.attr.hydfab]{rafts_retr_nhdp_comids_geom}
  #' @seealso \code{rafts_algo.utils.rafts_retr_nhdp_comids_geom_wrap}
  #' @export

  # Changelog/Contributions
  #. 2025-03-07 Originally created, GL
  #. 2025-06-04 bugfix, idxs_gage_ids logic, GL
  sf_comid_in <- proc.attr.hydfab::read_rafts_retr_gpkg(path_save_gpkg)
  ...
  if(!base::is.null(sf_comid_in)){ # Read in
    ...
    if(base::length(idxs_gage_ids) == base::length(gage_ids)){
      ...
      sf_comid_need <- proc.attr.hydfab::rafts_retr_nhdp_comids_geom(
        gage_ids=need_gids,
    ...
    sf_comid <- proc.attr.hydfab::rafts_retr_nhdp_comids_geom(gage_ids = gage_ids,
                                                           featureSource=featureSource,
                                                           featureID=featureID) %>%
      sf::st_as_sf(crs=epsg)
...
rafts_retr_nhdp_comids_geom <- function(gage_ids,featureSource='nwissite',
  ...
  #' @seealso \link[proc.attr.hydfab]{proc_attr_read_gage_ids_fs}
  #' @seealso \link[proc.attr.hydfab]{rafts_retr_nhdp_comids_geom_wrap}
  #' @seealso \code{rafts_algo.utils.rafts_retr_nhdp_comids_geom}
...
std_attr_data_fmt <- function(attr_data){
  ...
  #'  }
  #'  Additionally, uses the internal function `read_rafts_attr_menu_config` to
  ...
  #' @seealso \link[proc.attr.hydfab]{read_rafts_attr_menu_config}
  #' @export
  # Changelog/Contributions
      ...
      if(dat_srce!="hfatl_vars"){ # hfatl_vars can be many things, but hydroatlas_attributes, camels_attributes, and usgs_attributes are standardized
        ...
        if(base::any(!attr_data_ls[[dat_srce]]$attribute %in% all_attrs)){
          ...
          str_problem <- glue::glue("The following entries are not attributes per the attribute menu and shall be removed\n",
                                    ...
                                    "Refer to proc.attr.hydfab::read_rafts_attr_menu_config for the allowable attributes defined in package config file.")
...
std_path_attrs <- function(comid, dir_db_attrs){
  ...
  #' @seealso \link[proc.attr.hydfab]{proc_attr_wrap}
  #' @seealso \code{rafts_algo.utils.rafts_read_attr_comid()} python function
...
io_attr_dat <- function(dt_new_dat,path_attrs,
  ...
  if(base::any(base::is.character(dt_cmbo$value))){
    character_msg <- glue::glue("PROBLEM: the value column is character class.
                         ...
                         the python rafts_read_attr_comid in the rafts_algo package.
                         Edit the rafts_read_attr_comid function and consider
                         one-hot encoding for RaFTS")
    logr::log_print(character_msg,
                    level = "WARN")
    warning(character_msg)
  } else {
    # Enforcing float64 needed for rafts_algo.utils.rafts_read_attr_comid
...
retr_comids <- function(gage_ids,featureSource,featureID,dir_db_attrs,
  ...
  #   with sf_comid from rafts_retr_nhdp_comids_geom_wrap, GL
  ...
  if(base::is.null(path_save_gpkg)){
    ...
    sf_comid <- proc.attr.hydfab::rafts_retr_nhdp_comids_geom_wrap(
  ...
  for (i in 1:length(gage_ids)){ #
    ...
    if(nldi_feat$featureSource == 'comid'){ # A simple case that may not off
      ...
      #. rafts_retr_nhdp_comids_geom_wrap() exists. For now it serves a double-check.
...
proc_attr_gageids <- function(gage_ids,featureSource,featureID,Retr_Params,
  ...
  #' \link[proc.attr.hydfab]{grab_attrs_datasets_rafts_wrap}
  #' @param gage_ids array of gage_id values to be queried for catchment attributes
  ...
  #'  \item \code{path$dir_std_base} the location of user_data_std containing dataset that were standardized by \pkg{rafts_prep}.
  ...
  #' @param lyrs character. The layer names of interest from the hydrofabric gpkg. Default 'network'
  #' @param overwrite boolean. Should the hydrofabric cloud data acquisition be redone and overwrite any local files? Default FALSE.
  #' @seealso \link[proc.attr.hydfab]{grab_attrs_datasets_rafts_wrap} loops over multiple datasets in calling this gage_id retrieval
  ...
  #.  2025-08-05 move gen_ds_gpkg from grab_attrs_datasets_rafts_wrap into here, GL
  ...
  # Note that the db update of new locations happens in grab_attrs_datasets_rafts_wrap
...
std_path_dataset <- function(dir_dataset, ds_filenames = ''){
  #' @title Return filepath of standardized rafts gage_id
  ...
  # ----  Read in a standard format filename and file type from rafts_prep ---- #
  # TODO make this more adaptable so that it doesn't depend on running python rafts_prep beforehand
...
proc_attr_read_gage_ids_fs <- function(dir_dataset, ds_filenames=''){
  #' @title Read in standardized rafts gage_id location identifiers
  #' @description Reads output generated using \pkg{rafts_prep} python package and
...
gen_ds_gpkg <- function(dir_db_gpkg, path_save_gpkg, gage_ids,epsg=4326){
  # --------------------- gpkg db prep for dataset ------------------------- #
  #' @title Generate dataset geopackage
  #' @description This implements a geopackage database containing all locations
  #' ever acquired using RaFTS and copies the relevant gage_id locations over
  #' to the dataset's directory. This is a .gpkg file read/write function and
  #' doesn't return any objects.
  #' @details Further processing inside proc_attr_gageids
  #' will retrieve any missing locations using NLDI queries, which can take a
  #' while! The geopackage database reduces the total NLDI queries needed for
  #' any dataset.
  #' @param dir_db_gpkg The directory containing the .gpkg database
  #' @param path_save_gpkg The path to the dataset-sepecific .gpkg
  #' @param gage_ids array of gage_id identifiers corresponding to the 'gage_id'
  #' column in the geopackage files
  #' @param epsg The EPSG code to use for the CRS; Default 4326, same as nhdplus.
  #' @seealso \link[proc.attr.hydfab]{grab_attrs_datasets_rafts_wrap}
  #' @seealso \link[proc.attr.hydfab]{update_gpkg_db}
  #' @export
  # Changelog/contributions
  ...
  if(base::file.exists(path_gpkg_all)){
    ...
    if(base::file.exists(path_save_gpkg)){
      ...
      sf_ds_write <- proc.attr.hydfab::std_write_geom_map_gpkg(sf_cmbo,path_save_gpkg,epsg=epsg)
    } else if (base::nrow(sf_ds)>0){
      sf_ds_write <- proc.attr.hydfab::std_write_geom_map_gpkg(sf_ds,path_save_gpkg,epsg=epsg)
    }
  }
}

update_gpkg_db <- function(dir_db_gpkg, path_save_gpkg, epsg=4326){
  #' @title Update the geopackage database with the dataset's gpkg
  #' @description This is a .gpkg file read/write function and doesn't return
  #'  any objects.
  #' @details \code{gen_ds_gpkg} Updates the dataset gpkg in the dataset dir
  #' using data that already exist in the gpgk database.
  #' \code{update_gpkg_db} is performed after retrieving missing gage_id
  #' locations, and takes any additions to the dataset gpkg and adds them
  #' to the gpkg database.
  #' @param dir_db_gpkg The directory containing the .gpkg database
  #' @param path_save_gpkg The path to the dataset-sepecific .gpkg
  #' @param epsg The EPSG code to use for the CRS; Default 4326, same as nhdplus.
  #' @seealso \link[proc.attr.hydfab]{gen_ds_gpkg}
  #' @seealso \link[proc.attr.hydfab]{grab_attrs_datasets_rafts_wrap}
  #' @export
  # Changelog/contributions
  #. 2025-07-25 originally created, GL
  ...
  if(!base::file.exists(path_gpkg_all)){ # Create the gpkg database
    ...
    sf_write <- proc.attr.hydfab::std_write_geom_map_gpkg(sf_cmbo,path_gpkg_all,epsg=epsg)
  }
}



grab_attrs_datasets_rafts_wrap <- function(Retr_Params,lyrs="network",overwrite=FALSE,
                                        path_save_gpkg_cstm=NULL){
  #' @title Grab catchment attributes from processed rafts input
  #' @description Wrapper function that acquires catchment attribute data from
  #' rafts processed input generated via \pkg{rafts_prep} package
  ...
  #'  \link[proc.attr.hydfab]{proc_attr_read_gage_ids_fs} - retrieves the gage_ids generated by \pkg{rafts_prep}
  ...
  #. 2025-07-24 Add gen_ds_gpkg and update_gpkg_db, GL
  ...
  for(ds in datasets){ # Looping by dataset
    ...
    # Retrieve the gage_ids, featureSource, & featureID from rafts_prep standardized output
    ...
    if(base::nrow(dat_loc)>0){
      ...
      if(base::is.null(path_save_gpkg_cstm)){
        logr::log_print("STRONGLY RECOMMENDED that user provide path_save_gpkg_cstm to
                proc.attr.hydfab::grab_attrs_datasets_rafts_wrap.", level = "WARN")
      } else {
        path_save_gpkg <- path_save_gpkg_cstm
      }
  ...
  # -------------------------- UPDATE GPKG DATABASE -------------------------- #
  for(ds in base::names(ls_path_save_gpkg)){
    path_save_gpkg <- ls_path_save_gpkg[[ds]]
    proc.attr.hydfab::update_gpkg_db(dir_db_gpkg=Retr_Params$paths$dir_db_gpkg,
                                     path_save_gpkg=path_save_gpkg, epsg=4326)
  }
...
check_attr_selection <- function(attr_cfg_path = NULL, vars = NULL, verbose = TRUE){
  ...
  # Read in the menu of attributes available through rafts
  dir_extdata <- system.file("extdata",package="proc.attr.hydfab")
  path_attr_menu <- base::file.path(dir_extdata, 'rafts_attr_menu.yaml')
...
std_path_miss_tfrm <- function(dir_db_attrs){
  ...
  #' @seealso \code{rafts_algo.tfrm_attrs.std_path_miss_tfrm} python package
  #' @export
...
uniq_id_loc_attr <- function(comids,attrs){
  ...
  #' @seealso \link[proc.attr.hydfab]{rafts_attrs_miss_mlti_wrap}
  uniq_cmbo <- paste0(comids,"_",attrs)
  return(uniq_cmbo)
}

rafts_attrs_miss_mlti_wrap <- function(path_attr_config){
  ...
  #'  from rafts_tfrm_attrs.py, and generated as a file by python function
  #'  \code{rafts_algo.tfrm_attr.write_missing_attrs}
  #' @param path_attr_config The file path to the attribute config file
  #' @seealso \code{rafts_algo.tfrm_attr.write_missing_attrs} python
  #' @seealso \code{rafts_attrs_miss.R} Rscript that calls this wrapper
  #' @export
  # Changelog / Contributions
