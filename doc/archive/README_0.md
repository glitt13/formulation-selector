...
----
Note 2026-07-31: This README describes the legacy form of RaFTS and original intent, which relied on retrieving watershed attributes from publicly available datasets using `proc.attr.hydfab`. As of 2026, that functionality has adapted to rather depend on the user providing the attribute datasets (ie the `hfATLAS` workflow). These instructions should still be applicable for legacy workflows.
...
### TLDR
 ...
 - Build a yaml config file `/sripts/eval_metrics/name_of_dataset_here/name_of_dataset_schema.yaml` [refer to this template](https://github.com/NOAA-OWP/rafts/blob/main/scripts/eval_ingest/xssa/xssa_schema.yaml)
 - Create a script that reads in the data and runs the standardization processing. [Example script here](https://github.com/NOAA-OWP/rafts/blob/main/scripts/eval_ingest/xssa/proc_xssa_metrics.py)
...
We track these tasks inside `rafts/scripts/eval_ingest/_name_of_raw_dataset_here_/`
#### 1. `data_schema.yaml`
