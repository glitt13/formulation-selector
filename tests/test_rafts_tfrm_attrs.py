...
class TestFsPrepProcAttrHydfabFsAlgo(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.path_attr_cfig = fsate.build_cfig_path(self.path_cfg_prep, "xssa_attr_config.yaml") #self.dir_tests / "config" / "xssa" / "xssa_attr_config.yaml"
        ...
        self.path_tfrm_cfig = fsate.build_cfig_path(self.path_cfg_prep, "xssatf_attrs_tform.yaml")
        self.path_fs_tfrm_attrs =  self.dir_tests.parent / "pkg" / "fs_algo" / "fs_algo" / "flow" / "fs_tfrm_attrs.py"
        
        
        # -------------- ALGO TRAIN/TEST ------------------------------------ #
        # Parse the algo config file:
        self.path_algo_cfg =fsate.build_cfig_path(self.path_cfg_prep, "xssatf_algo_config.yaml") 
        self.path_fs_proc_algo_viz = self.dir_tests.parent / "pkg" / "fs_algo" / "fs_algo" / "flow" / "fs_proc_algo_viz.py"
        self.algo_cfig = fsate.AlgoConfigParser(self.path_algo_cfg)
    ...
    def test_04_fs_proc_algo_viz(self):
        ...
        dict_resp_gdf = fsate.combine_resp_gdf_comid_wrap(dir_std_base=self.dir_std_base,
                        ds= self.dataset, path_attr_config=self.path_attr_cfig)
        metrics = dict_resp_gdf['dat_resp'].attrs['metric_mappings'].split('|')
        ls_joblib_paths = [fsate.std_algo_path(self.dir_out_alg_ds, algo, metric, self.dataset) for algo in algos for metric in metrics]
        ...
        ls_anls_paths = [fsate.std_test_pred_obs_path(self.dir_out_anlys_base,self.dataset,metr) for metr in metrics]
    ...
    def test_06_fs_pred(self):
        ...
        self.path_pred_cfg = fsate.build_cfig_path(self.path_cfg_prep, "xssatf_pred_config.yaml") #self.dir_tests / "config" / "xssa" / "xssa_pred_config.yaml"
        pred_cfg = fsate.PredConfigParser(self.path_pred_cfg)
        ...
        files_pred_std = [fsate.std_pred_path(self.dir_out,algo,metr, self.dataset)
