...
python3 "${DIR_PY}rafts_proc_algo_viz.py" "${DIR_CONFIG}xssaus_algo_config_uncn.yaml"
...
python3 "${DIR_PY}rafts_pred_algo.py" "${DIR_CONFIG}xssaus_pred_config_uncn.yaml"

# 4.4 Map the predictions (static map)
echo "Plotting the process predictions on static map"
python3 "${DIR_CONFIG}rafts_proc_viz_xssaus.py" "${DIR_CONFIG}xssaus_pred_config_uncn.yaml"
