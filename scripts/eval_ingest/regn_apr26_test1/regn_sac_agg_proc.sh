...
uv run --project "${DIR_REPO}/pkg" python "${DIR_CONFIG}/prep_regn_test_agg.py" "${DIR_CONFIG}/regn_sac_agg_prep_config.yaml" || {
...
uv run --project "${DIR_REPO}/pkg" python "${DIR_PREP}/hfatlas_to_rafts_prep.py" \
    --path_prep_config "${DIR_CONFIG}/regn_sac_agg_prep_config.yaml" \
    --name_attr_config "regn_sac_attr_config.yaml" || {
...
uv run --project "${DIR_REPO}/pkg" python "${DIR_PY}/rafts_proc_algo_pool.py" "${DIR_CONFIG}/regn_sac_algo_config_uncn.yaml" --chunk_size 4 || {
...
uv run --project "${DIR_REPO}/pkg" python "${DIR_PY}/rafts_pred_algo.py" "${DIR_CONFIG}/regn_sac_pred_config_uncn.yaml" || {
...
uv run --project "${DIR_REPO}/pkg" python "${DIR_PY}/rafts_map_pred_hfatl.py" "${DIR_CONFIG}/regn_sac_pred_config_uncn.yaml" || {
