...
# Instructions:
# Make this script executable using: chmod +x regn_top_agg_proc.sh
# Run by calling in terminal: ./regn_top_agg_proc.sh
...
uv run --project "${DIR_REPO}/pkg" python "${DIR_CONFIG}/prep_regn_test_agg.py" "${DIR_CONFIG}/regn_top_agg_prep_config.yaml" || {
...
uv run --project "${DIR_REPO}/pkg" python "${DIR_PREP}/hfatlas_to_rafts_prep.py" \
    --path_prep_config "${DIR_CONFIG}/regn_top_agg_prep_config.yaml" \
    --name_attr_config "regn_top_attr_config.yaml" || {
...
uv run --project "${DIR_REPO}/pkg" python "${DIR_PY}/rafts_proc_algo_pool.py" "${DIR_CONFIG}/regn_top_algo_config_uncn.yaml" --chunk_size 4 || {
...
uv run --project "${DIR_REPO}/pkg" python "${DIR_PY}/rafts_pred_algo.py" "${DIR_CONFIG}/regn_top_pred_config_uncn.yaml" || {
...
uv run --project "${DIR_REPO}/pkg" python "${DIR_PY}/rafts_map_pred_hfatl.py" "${DIR_CONFIG}/regn_top_pred_config_uncn.yaml" || {
