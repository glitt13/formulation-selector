...
#' @examples
#' \dontrun{Rscript gen_pred_locs_xssa.R --path_cfig_pred "{home_dir}/git/formulation-selector/scripts/eval_ingest/xssa_us/xssaus_pred_config.yaml".yaml"
#' }
...
main <- function(){
  ...
  if(FALSE){
    ...
    df_huc08 <- read.csv("~/noaa/regionalization/data/rfc_locs/comids_highest_hf_hydroseq.csv")
    col_comid_huc08 <- "hf_id"
    path_gpkg_comids <- "~/noaa/regionalization/data/analyses/basin_selection/terminal_nonhuc08_with_comids.gpkg"
    path_gpkg_comids <- "~/noaa/regionalization/data/analyses/basin_selection/terminal_locs/nhdp_cat_line_out_tnx_above10sqkm.gpkg"
